...
\end{equation}
where $L_{min}$ is the characteristic length of the smallest element in the mesh and $c$ is the local speed of sound in the material ($c = \sqrt{E/\rho}$). For this study, the 2~mm mesh resolution at the helmet crown dictated a critical global time-step of approximately $0.42~\mu s$. Consequently, simulating the brief 10 millisecond physical impact event required over 23,000 computational cycles, highlighting the heavy computational expense inherent to explicit dynamics.
...
\cite{lsdyna2021, belytschko2013}. This approach was applied 
in benchmark helmet FEA studies by Liu et al. \cite{liu2003} and 
...
In this investigation, a penalty scale factor ($f_s$) of 0.1 was applied. This value balances the enforcement of kinematic constraints (preventing unphysical mesh overlap) with thermodynamic stability. Higher penalty factors artificially stiffen the contact interface, potentially driving the required explicit time-step below the Courant-Friedrichs-Lewy (CFL) stability limit and inducing catastrophic high-frequency noise.
contact interface, potentially driving the required explicit time-step below the Courant-Friedrichs-Lewy (CFL) stability limit and inducing catastrophic high-frequency noise.
